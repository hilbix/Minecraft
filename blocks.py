...
def _init(M):
    ...
    M.crack_textures = CrackTextureBlock()

    # nether
    M.nether_block = NetherrackBlock()
    M.netherore_block = NetherOreBlock()
    M.soulsand_block = SoulsandBlock()
    # overworld blocks

    M.air_block = AirBlock()
    M.grass_block = GrassBlock()
    M.sand_block = SandBlock()
    M.brick_block = BrickBlock()
    M.stone_block = StoneBlock()
    M.dirt_block = DirtBlock()
    M.lamp_block = LampBlock()
    M.bed_block = BedrockBlock()
    M.water_block = WaterBlock()
    M.craft_block = CraftTableBlock()
    M.sandstone_block = SandstoneBlock()
    M.quartz_block = QuartzBlock()
    M.stonebrick_block = StonebrickBlock()
    M.birchwoodplank_block = BirchWoodPlankBlock()
    M.junglewoodplank_block = JungleWoodPlankBlock()
    M.oakwoodplank_block = OakWoodPlankBlock()
    M.snowgrass_block = SnowGrassBlock()
    M.oakwood_block = OakWoodBlock()
    M.oakleaf_block = OakLeafBlock()
    M.oakbranch_block = OakBranchBlock()
    M.junglewood_block = JungleWoodBlock()
    M.jungleleaf_block = JungleLeafBlock()
    M.birchwood_block = BirchWoodBlock()
    M.birchleaf_block = BirchLeafBlock()
    M.cactus_block = CactusBlock()
    M.coalore_block = CoalOreBlock()
    M.ironore_block = IronOreBlock()
    M.goldore_block = GoldOreBlock()
    M.diamondore_block = DiamondOreBlock()
    M.melon_block = MelonBlock()
    M.stoneslab_block = StoneSlabBlock()
    M.clay_block = ClayBlock()
    M.cobble_block = CobbleBlock()
    M.mossycobble_block = MossyCobbleBlock()
    M.bookshelf_block = BookshelfBlock()
    M.furnace_block = FurnaceBlock()
    M.farm_block = FarmBlock()
    M.glass_block = GlassBlock()
    M.gravel_block = GravelBlock()
    M.tallcactus_block = TallCactusBlock()
    M.pumpkin_block = PumpkinBlock()
    M.torch_block = TorchBlock()
    M.yflowers_block = YFlowersBlock()
    M.cobblefence_block = CobbleFenceBlock()
    M.snow_block = SnowBlock()
    M.meta_block = MetaBlock()
    M.chest_block = ChestBlock()
    M.cake_block = CakeBlock()
    # wool
    M.blackwool_block = BlackWoolBlock()
    M.redwool_block = RedWoolBlock()
    M.greenwool_block = GreenWoolBlock()
    M.brownwool_block = BrownWoolBlock()
    M.bluewool_block = BlueWoolBlock()
    M.purplewool_block = PurpleWoolBlock()
    M.cyanwool_block = CyanWoolBlock()
    M.lightgreywool_block = LightGreyWoolBlock()
    M.greywool_block = GreyWoolBlock()
    M.pinkwool_block = PinkWoolBlock()
    M.limewool_block = LimeWoolBlock()
    M.yellowwool_block = YellowWoolBlock()
    M.lightbluewool_block = LightBlueWoolBlock()
    M.magentawool_block = MagentaWoolBlock()
    M.orangewool_block = OrangeWoolBlock()
    M.whitewool_block = WhiteWoolBlock()

    # carpet
    M.blackcarpet_block = BlackCarpetBlock()
    M.redcarpet_block = RedCarpetBlock()
    M.greencarpet_block = GreenCarpetBlock()
    M.browncarpet_block = BrownCarpetBlock()
    M.bluecarpet_block = BlueCarpetBlock()
    M.purplecarpet_block = PurpleCarpetBlock()
    M.cyancarpet_block = CyanCarpetBlock()
    M.lightgreycarpet_block = LightGreyCarpetBlock()
    M.greycarpet_block = GreyCarpetBlock()
    M.pinkcarpet_block = PinkCarpetBlock()
    M.limecarpet_block = LimeCarpetBlock()
    M.yellowcarpet_block = YellowCarpetBlock()
    M.lightbluecarpet_block = LightBlueCarpetBlock()
    M.magentacarpet_block = MagentaCarpetBlock()
    M.orangecarpet_block = OrangeCarpetBlock()
    M.whitecarpet_block = WhiteCarpetBlock()

    M.rose_block = RoseBlock()
    M.reed_block = ReedBlock()
    M.potato_block = PotatoBlock()
    M.carrot_block = CarrotBlock()
    M.diamond_block = DiamondBlock()
    M.gold_block = GoldBlock()
    M.iron_block = IronBlock()
    M.stonebrickcracked_block = CrackedStonebrickBlock()
    M.stonebrickmossy_block = MossyStonebrickBlock()
    M.quartzcolumn_block = ColumnQuartzBlock()
    M.quartzchiseled_block = ChiseledQuartzBlock()
    M.quartzbrick_block = QuartzBrickBlock()
    M.ice_block = IceBlock()
    M.emeraldore_block = EmeraldOreBlock()
    M.lapisore_block = LapisOreBlock()
    M.rubyore_block = RubyOreBlock()
    M.sapphireore_block = SapphireOreBlock()
    M.fern_block = TallGrassBlock()
    M.wheat_crop_block = WheatCropBlock()
    #wild grass of different heights
    M.wildgrass0_block = Grass0Block()
    M.wildgrass1_block = Grass1Block()
    M.wildgrass2_block = Grass2Block()
    M.wildgrass3_block = Grass3Block()
    M.wildgrass4_block = Grass4Block()
    M.wildgrass5_block = Grass5Block()
    M.wildgrass6_block = Grass6Block()
    M.wildgrass7_block = Grass7Block()
    M.desertgrass_block = DesertGrassBlock()
    M.deadbush_block = DeadBushBlock()
